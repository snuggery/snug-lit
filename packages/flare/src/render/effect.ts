...
import type {AfterRenderOptions, AfterRenderRef} from "./hooks.js";
import {AfterRenderManager, AfterRenderSequence} from "./manager.js";
...
class AfterRenderEffectSequence extends AfterRenderSequence {
	...
	constructor(
		...
		super(manager, [undefined, undefined, undefined, undefined], false, host);
		...
		for (let phase = 0; phase < 3; phase++) {
			...
			const node = Object.create(
				AFTER_RENDER_PHASE_EFFECT_NODE,
			) as AfterRenderPhaseEffectNode;
			node.sequence = this;
			node.phase = phase as typeof node.phase;
 ...
 */
export type ɵFirstAvailableSignal<T extends unknown[]> = T extends [
	infer H,
	...infer R,
]
	? [H] extends [never]
...
export function afterRenderEffect<E = never, W = never, M = never>(
	spec: {
		...
		write?: (
			...args: [...ɵFirstAvailableSignal<[E]>, EffectCleanupRegisterFn]
		) => W;
		mixedReadWrite?: (
			...args: [...ɵFirstAvailableSignal<[W, E]>, EffectCleanupRegisterFn]
		) => M;
		read?: (
			...args: [...ɵFirstAvailableSignal<[M, W, E]>, EffectCleanupRegisterFn]
		) => void;
	},
